...
namespace ITRTelemetry.Controllers
{
    ...
    [ApiController]
    public class RulesandregulatoryController : ControllerBase
    {
        ...
        public RulesandregulatoryController(MySqlDBContext mySqlDBContext, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        [Route("api/rulesandregulatory/GetrulesandregulatoryDetails")]
        [HttpGet]
        ...
        public IEnumerable<object> GetrulesandregulatoryDetails()
        {
            ...
            var act = from act_rule_regulatory in mySqlDBContext.Rulesandregulatorymodels
                      join actregulatory in mySqlDBContext.Actregulatorymodels on act_rule_regulatory.actregulatoryid equals actregulatory.actregulatoryid
        ...
        [Route("api/rulesandregulatory/GetrulesandregulatoryByID/{actid}")]
        [HttpGet]
        public IEnumerable<object> GetrulesandregulatoryByID(int actid)
        {
            try
            {
                //return this.mySqlDBContext.Actregulatorymodels.Where(x => x.status == "Active").ToList();
        ...
        [Route("api/rulesandregulatory/removerulesandregulatory/{act_rule_regulatory_file_id}")]
        [HttpPost]
        public void removerulesandregulatory(int act_rule_regulatory_file_id)
        {
            try
            {


                var currentClass = new ActRuleregulatoryfilemodel { act_rule_regulatory_file_id = act_rule_regulatory_file_id };
        ...
        [Route("api/rulesandregulatory/GetbareactReferenceByID/{actid}")]
        [HttpGet]
        public IEnumerable<object> GetBareActReference(int actid)
        {
            var bareactIds = mySqlDBContext.Actregulatoryfilemodels
                .Where(bareact => bareact.status == "Active" && bareact.actregulatoryid == actid)
                .Select(bareact => bareact.bare_act_id)
                .ToList();

            var combinedResults = new List<object>();

            foreach (var bareactid in bareactIds)
            {
                var filesForbareactId = mySqlDBContext.Actregulatoryfilemodels
                    .Where(files => files.bare_act_id == bareactid)
                    .ToList();

                if (filesForbareactId.Any())
                {
                    var links = filesForbareactId
        ...
        [Route("api/ActRegulatory/InsertRuleActRegulatory")]
        [HttpPost]
        public async Task<IActionResult> InsertRuleActRegulatory()
        {
            ...
            try
            {
                ...
                var Maxruleid = this.mySqlDBContext.Rulesandregulatorymodels.Where(d => d.IsImportedData == "No").Max(d => (int?)d.act_rule_regulatory_id) ?? 0;
                ...
                var globalactId = GenerateGlobalActId(con, int.Parse(actregulatoryid));
                ...
                {
                    var fileUploadModel = new ActRuleregulatoryfilemodel
                    {
                    ...
                    var Maxrulefileid = this.mySqlDBContext.ActRuleregulatoryfilemodels.Where(d => d.IsImportedData == "No").Max(d => (int?)d.act_rule_regulatory_file_id) ?? 0;

                    var newActfileRegulatoryId = Maxrulefileid + 1;
                    string insertSubTableQuery = "INSERT INTO risk.act_rule_regulatory_file (act_rule_regulatory_file_id,act_rule_regulatory_id, global_rule_id, filecategory, filepath, status, created_date,file_name,IsImportedData) " +
                                          "VALUES (@act_rule_regulatory_file_id,@act_rule_regulatory_id, @global_rule_id, @fileCategory, @filePath, @status, @createddate,@file_name,@IsImportedData)";
        ...
        private string GenerateGlobalActId(MySqlConnection con, int actregulatoryid)
        {
            ...
            string getMaxIdQuery = "SELECT MAX(CAST(SUBSTRING(global_actId, LENGTH(global_actId) - 3) AS UNSIGNED)) FROM risk.act_regulatorymaster WHERE actregulatoryid = @actregulatoryid";
            ...
            using (MySqlCommand getMaxIdCommand = new MySqlCommand(getMaxIdQuery, con))
            {
                ...
                try
                {
                    ...
                    string getSubNumberQuery = $"SELECT MAX(CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(global_rule_id, '.', -1), '.', 1) AS UNSIGNED)) AS max_cc_subnumber " +
                                                $"FROM risk.act_rule_regulatory WHERE global_rule_id LIKE 'CC{maxSerialNo:D4}.%'";

                    using (MySqlCommand getSubNumberCommand = new MySqlCommand(getSubNumberQuery, con))
                    {
                        object maxCCSubNumberObj = getSubNumberCommand.ExecuteScalar();
                        int maxCCSubNumber = 0;

                        if (maxCCSubNumberObj != DBNull.Value && maxCCSubNumberObj != null)
                        {
                            maxCCSubNumber = Convert.ToInt32(maxCCSubNumberObj);
                            maxCCSubNumber++; // Increment the sub-number by 1
                        }
                        else
                        {
                            maxCCSubNumber = 1; // If no existing records, set the sub-number to 1
                        }

                        gActId = $"CC{maxSerialNo:D4}.{maxCCSubNumber:D3}"; // Ensure it's formatted correctly
                    }
                }
                catch (Exception ex)
                {
        ...
        [Route("api/actruleregulation/GetactruleandprocudreDetails/{ruleid}")]
        [HttpGet]

        public IEnumerable<object> GetactruleandprocudreDetails(int ruleid)
        {
            // Fetch initial data using LINQ query
            var result = (from act_rule_regulatory in mySqlDBContext.Rulesandregulatorymodels
                          join actrulefilerepositiory in mySqlDBContext.ActRuleregulatoryfilemodels
                          ...
                          join actregulatory in mySqlDBContext.Actregulatorymodels
                          ...
                          join category in mySqlDBContext.catageoryoflawmodels
                          ...
                          join Jurisdiction in mySqlDBContext.Jurisdictionmodels
                              on act_rule_regulatory.category_of_law_ID equals Jurisdiction.jurisdiction_category_id into Jurisdiction_join
                          from Jurisdiction in Jurisdiction_join.DefaultIfEmpty()
                          join jurisdiction_location in mySqlDBContext.JurisdictionLocationModels
                          ...
                          join regulator in mySqlDBContext.RegulatoryAuthorityModels
                          ...
                          join country in mySqlDBContext.CountryModels
                              on act_rule_regulatory.id equals country.id into country_join
                          from country in country_join.DefaultIfEmpty()
                          join state in mySqlDBContext.StateModels
                              on act_rule_regulatory.State_id equals state.id into state_join
                          from state in state_join.DefaultIfEmpty()
                          join tbluser in mySqlDBContext.usermodels
                              on actregulatory.createdBy equals tbluser.USR_ID into tbluser_join
                          from tbluser in tbluser_join.DefaultIfEmpty()
                          join law_type in mySqlDBContext.LawTypeModels
                          ...
                          join user in mySqlDBContext.usermodels
                              on act_rule_regulatory.updatedby equals user.USR_ID into userJoin
                          ...
                          select new
                          {
                            ...
                            //  countryId = jurisdiction_location.jurisdiction_country_id,
                             // stateId = jurisdiction_location.jurisdiction_state_id,
                              ...
                              isImportedData = act_rule_regulatory.IsImportedData,
        ...
        [Route("api/actruleregulations/GetactruleregulationsDetailsnyid/{ruleid}")]
        [HttpGet]

        public IEnumerable<object> GetactruleregulationsDetailsnyid(int ruleid)
        {
            var result = (from act_rule_regulatory in mySqlDBContext.Rulesandregulatorymodels
                          join actrulefilerepositiory in mySqlDBContext.ActRuleregulatoryfilemodels
                              on act_rule_regulatory.act_rule_regulatory_id equals actrulefilerepositiory.act_rule_regulatory_id into act_rule_regulatory_join
        ...
        [Route("api/actruledownload/actruleDownLoadFiles")]
        [HttpGet]
        public async Task<IActionResult> ruleDownloadFile(string filePath)
        {
            try
            {
        ...
        [Route("Api/actruleregulations/UpdateActRuleRegulatory")]
        [HttpPost]

        public async Task<IActionResult> UpdateActRuleRegulatory([FromQuery] int act_rule_regulatory_id)
        {
            try
            {
                var Maxrulefileid = this.mySqlDBContext.ActRuleregulatoryfilemodels.Where(d => d.IsImportedData == "No").Max(d => (int?)d.act_rule_regulatory_file_id) ?? 0;
                ...
                var newActfileRegulatoryId = Maxrulefileid + 1;
                var rule = await mySqlDBContext.Rulesandregulatorymodels.FirstOrDefaultAsync(a => a.act_rule_regulatory_id == act_rule_regulatory_id);
                ...
                //   rule.State_id = int.Parse(formCollection["State_id"]);
                //   rule.jurisdiction_location_id = int.Parse(formCollection["jurisdiction_location_id"]);
                ...
                // Handle Weblinks
                var existingWeblinks = await mySqlDBContext.ActRuleregulatoryfilemodels
                    .Where(f => f.act_rule_regulatory_id == act_rule_regulatory_id && f.filecategory == "Weblink" && f.status == "Active")
                    .ToListAsync();
                ...
                foreach (var link in newWeblinks)
                {
                    ...
                    {
                        mySqlDBContext.ActRuleregulatoryfilemodels.Add(new ActRuleregulatoryfilemodel
                        {
                ...
                var existingFiles = await mySqlDBContext.ActRuleregulatoryfilemodels
                    .Where(f => f.act_rule_regulatory_id == act_rule_regulatory_id && f.filecategory == "FileAttach" && f.status == "Active")
                    .ToListAsync();
