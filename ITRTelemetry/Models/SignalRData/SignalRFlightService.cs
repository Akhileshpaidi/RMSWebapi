...
{
    public class SignalRFlightService
    {
        private IHubContext<SignalRFlightDataHub> hubContext { get; set; }
        ...
        public SignalRFlightService(IHubContext<SignalRFlightDataHub> hubCtx)
        {
            hubContext = hubCtx;
        }
        public IEnumerable<ParameterData> GetAllData(int ParameterTypeID, string FID, int frequency)
        {
            ParamTypeID = ParameterTypeID;
            FlightID = FID;
            tickData = GenerateTestData();
            lastTickIndex = tickData.Length - 1;
            timer = new Timer(Update, null, 100, frequency);
