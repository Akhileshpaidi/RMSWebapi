...
{
    public class OfflineChartSignalRService
    {
        private IHubContext<OfflineChartSignalRHub> hubContext { get; set; }
        ...
        private Timer timer;
        int count = 0;
        int SystemID, FlightID, MissionID, Frequency, Year, ParameterIDX, ParameterIDY, MINVal, MAXVal;
        string MissionStatus;
        MySqlConnection con;
        DataTable dt = new DataTable();
        DataTable dtp = new DataTable();
        public OfflineChartSignalRService(IHubContext<OfflineChartSignalRHub> hubCtx)
        {
        ...
        public IEnumerable<PlayBackParametersData> GetAllData()
        {
            ...
            for (var i = lastTickIndex; data.Count < 10; i--)
            {
                data.Add(tickData[i]);
                if (i == 0)
                {
        ...
        public void getOfflineDisplayModeDetails()
        {
            ...
            {
                con.Open();
                MySqlCommand cmd1 = new MySqlCommand("SELECT * FROM itr.playbackmodeoffline ORDER BY PlaybackmodeofflineID DESC LIMIT 2", con);
                ...
                if (dt1.Rows.Count > 1)
                {
                    ...
                    ParameterIDX = Convert.ToInt32(dt1.Rows[0]["ParameterID"].ToString());
                    ParameterIDY = Convert.ToInt32(dt1.Rows[1]["ParameterID"].ToString());
